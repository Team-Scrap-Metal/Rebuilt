...
public class DrumIOSpark implements DrumIO {
    ...
    public DrumIOSpark() {
        drumMotor = new SparkMax(DrumConstants.CAN_ID, MotorType.kBrushless);
        drumFollowerMotor = new SparkMax(DrumConstants.FOLLOWER_CAN_ID, MotorType.kBrushless);
        drumEncoder = drumMotor.getEncoder();

        var motorConfig = new SparkMaxConfig();
        var followerConfig = new SparkMaxConfig();
        motorConfig
            .inverted(DrumConstants.INVERTED)
            .idleMode(IdleMode.kCoast)
            .smartCurrentLimit(DrumConstants.CURRENT_LIMIT);
        followerConfig
            .apply(motorConfig)
            .follow(drumMotor, DrumConstants.FOLLOWER_INVERTED);
